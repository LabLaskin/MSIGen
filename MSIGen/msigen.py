...
from MSIGen.raw import MSIGen_raw
from MSIGen.mzml import MSIGen_mzml
from MSIGen.D import MSIGen_D
...
class MSIGen_base(object):
    ...
    def __new__(cls, example_file, mass_list_dir, *args, **kwargs):
        """Creates subclass for the specific file extension."""
        if cls is MSIGen_base:
            file_ext = cls.get_file_extension(example_file)
            if file_ext == '.d':
                return MSIGen_D(example_file, mass_list_dir, *args, **kwargs)
            elif file_ext == '.mzml':
                return MSIGen_mzml(example_file, mass_list_dir, *args, **kwargs)
            elif file_ext == '.raw':
                return MSIGen_raw(example_file, mass_list_dir, *args, **kwargs)
            else:
                raise Exception("Invalid file extension. Supported file extensions are: '.d', '.mzml', '.raw'")
    ...
    def get_metadata_and_params(self, **kwargs):
        ...
        if self.is_jupyter and (not self.testing):
    ...
    def display_mass_list(self):
        ...
        mass_list = deepcopy(self.mass_list)
        a = mass_list[1:]
    ...
    def get_image_data(self, **kwargs):
        ...
        self.metadata, self.pixels = self.load_files()
    ...
    def get_PeakCountsPerFilter(self, filters_info):
        ...
        if len(filters_info) == 5:
            ...
            is_mob = True
        MS1_list, MS1_mob_list, MS1_polarity_list, prec_list, frag_list, MS2_mob_list, MS2_polarity_list, mass_list_idxs = self.mass_lists
    ...
    def save_pixels(self, metadata, pixels, MSI_data_output=None, file_format = None, compress = False, gui = True):
        if MSI_data_output is not None:
            self.output_file_loc = MSI_data_output
